...
public class BlackjackProject {     // <----------CONTROLLERIN METODIT LÖYTYY TÄÄLTÄ ("Controller classi" = BlackjackProject class)
    ...
    public void setBankaccounts(Bankaccount playeraccount, Bankaccount dealeraccount, Bankaccount thePot){          // asettaa bankaccoiunts oliot
    ...
    public void setHandsToController(Hand playerhand, Hand dealerhand){     // asetetaan playerhand ja dealerhand oliiot
    ...
    public int evaluateNaturalBlackJackParameters(Hand diilerinkäsi, Hand pelaajankäsi){        // parametrillinen evaluateNaturalBlackJack metodi, liittyy muistaakseni sidegamessa tehtyihin tarkastuksiin, koska tulee uusi mahdollisuus saada natural blackjack
        if (pelaajankäsi.handValueOf()==21){
            if (pelaajankäsi.handValueOf()==21 && diilerinkäsi.handValueOf()==21){
                System.out.println("both have natural Blackjack, the game is a draw ");
                theView.printHandValues(diilerinkäsi, pelaajankäsi);
                return 1;
            }
            else{
                System.out.println("player has natural Blackjack, the player wins ");
                theView.printHandValues(diilerinkäsi, pelaajankäsi);
                return 2;
            }
        }
...
//            theView.printString(diilerhänd.toString());
//            diilerhänd.clearHand();           //händejä ei saa clearata vielä 
//            playerinkäsi.clearHand();
            return 0;
        }
        if (playerinkäsi.handValueOf() > 21) {                  // if player busts by going over 21, player  loses the hand
//            theView.printHandValues(diilerhänd, playerinkäsi);
              System.out.println("player busts and dealer wins  the hand ");
////            diilerhänd.clearHand();
//            playerinkäsi.clearHand();
            return 0;
        } else {                                           // if the player DID NOT bust by going over 21, we go to this else-branch to evaluate further
//            theView.printString("the dealer has: ");
//            theView.printString(diilerhänd.toString());
//            theView.printString("dealerhand is valued at " + diilerhänd.handValueOf());
...
//            }
            ...
            //    dealerhand.handValueOf();      // paluuarvoa ei hyödynnetä tässä metodissa MUTTA JOKAISEN HITTAUKSEN JÄLKEEN jälkeen pitää tarkistaa uusi pelikorttien tilanne ässien arvon uudelleen laskemista varten. Ässien arvo ei ole fixattu missään vaiheessa peliä oikeastaan, vaan jos ässä oli alussa 11 arvoinen, niin seuraavan kortin tultua, se ässä voi palata ykkösen arvoiseksi
              if(diilerhänd.handValueOf()>=17){
//                  theView.printString(diilerhänd.toString());
//                System.out.println("dealerhand is valued at " + diilerhänd.handValueOf());
                break;
              }
//            theView.printString(diilerhänd.toString());
//                System.out.println("dealerhand is valued at " + diilerhänd.handValueOf());
           ...
           // int dealerHandSum=diilerhänd.dealerHitLoop(diilerhänd);
...
//                theView.printHandValues(dealerhand, sidehand);
//                System.out.println(" dealer has natural BLACKJACK, dealer wins ");
////                dealerhand.clearHand();                           // TÄRKEÄ HUOM!!! handejä ei saa clearata vielä tässä vaiheessa koska metodi palauttaa paluuarvona vain resultit 
////                sidehand.clearHand();                                 // kun pelaaja standasi korteillansa ==> sidehandia JA dealerhandia ei saa vielä clearata koska maingamea ei ole vielä pelattu
//                System.out.println("sidegame ends, Dealer wins sidegame");          //   ==> sidehandia ja dealerhandia tarvitaan vielä loppuvaiheessa kun printataan sidegamen ja maingamen tulokset ja saadut kortit.
                return 0;
            }
            
//            else if (dealerhandStartValue >= 17 && dealerhandStartValue != 21){     // tässsä elif haara vain viestin printtaukseen perustuen dealerin alkukorttien kokoon
//                System.out.println("dealer stands ");
//            }
//            else{                                   // else haara tässä vain printtaukseen,
...
//                    System.out.println("sidegame ends, player wins sidegame ");
////                    dealerhand.clearHand();
////                    sidehand.clearHand();
                    return 2;
                }
...
////                    dealerhand.clearHand();
////                    sidehand.clearHand();
//                    System.out.println("sidegame ends, sidegame is a draw ");
                    return 1;
                }
...
////                    dealerhand.clearHand();
////                    sidehand.clearHand();
//                    System.out.println("sidegame ends, dealer wins sidegame ");
                    return 0;
                }
...
//                playerinkäsi.clearHand();
    ...
    public int playerHitEvaluate() {            // tämä on erittäin iso metodi, ja tämä metodi evaluoi kokonaisuudessaan PLAYER HITS extra cards playerin valinnan, että mitä tapahtui yhden blackjack rundin aikana, tuliko tasapeli, voitto vai tappio. Metodin paluuarvo on integer ja 2== voitto 1 == tasapeli 0== tappio 
        ...
        if (playerhand.handValueOf() > 21) {                  // if player busts by going over 21, player  loses the hand
            ...
            //    dealerhand.handValueOf();      //  LOOPISSA JOKAISEN HITTAUKSEN JÄLKEEN jälkeen pitää tarkistaa uusi pelikorttien tilanne ässien arvon uudelleen laskemista varten. Ässien arvo ei ole fixattu missään vaiheessa peliä oikeastaan, vaan jos ässä oli alussa 11 arvoinen, niin seuraavan kortin tultua, se ässä voi palata ykkösen arvoiseksi
            ...
            while (dealerhand.handValueOf() < 17 && playerhand.handValueOf() <= 21) {       // dealer keeps hitting until his handvalue is big enough
              ...
              if(dealerhand.handValueOf()>=17){
                ...
                theView.printString(dealerhand.toString());     // printtaa loopissa esim pikkukortteja ja niiden arvoja.
                System.out.println("dealerhand is valued at " + dealerhand.handValueOf());
            }
